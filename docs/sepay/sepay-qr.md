...
> **Hai kiểu tích hợp:** Monorepo này dùng **Payment Gateway** — tạo đơn rồi redirect khách lên trang thanh toán SePay (`payment-flow-sepay.md`). Bài dưới đây mô tả kiểu **nhúng ảnh QR** qua `qr.sepay.vn/img` trên **trang của merchant**. Đừng trộn **redirect gateway** với **mã QR tĩnh / nhúng riêng** trên cùng một màn hình checkout — dễ trùng luồng và gây nhầm cho khách.
