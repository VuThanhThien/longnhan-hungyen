...
export const mediaApi = {
  async list(params?: { page?: number; limit?: number; folder?: string }) {
    const response = await httpClient.get<PaginatedResponse<MediaItem>>('/media', {
      params: {
        page: params?.page ?? 1,
  ...
  async listFolders() {
    const response = await httpClient.get<{ data: MediaFolder[] } | MediaFolder[]>('/media/folders');
    return Array.isArray(response.data) ? response.data : (response.data.data ?? []);
  },

  async createFolder(name: string) {
    const response = await httpClient.post<{ data: MediaFolder } | MediaFolder>('/media/folders', { name });
  ...
  async upload(file: File, folder = 'general') {
    ...
    const response = await httpClient.post<{ data: MediaItem }>(`/media/upload?folder=${folder}`, payload);
    return response.data.data;
  },
