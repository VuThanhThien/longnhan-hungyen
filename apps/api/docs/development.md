...
- `MODULES_SET`: The deployment mode. Options: `monolith` (default, all modules), `api` (API only), `background` (background jobs only).
...
# Start the development server with file watcher (same as start:dev)
pnpm dev
...
```bash
# Lint all files and fix violations automatically
pnpm lint
