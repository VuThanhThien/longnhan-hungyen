...
Contains all API modules and feature implementations.
...
- **HomeModule** — Home/index endpoint
- **PostModule** — Blog posts (legacy, will be replaced by ArticlesModule)
- **ProductsModule** — E-commerce products catalog (`GET /v1/products`, `GET /v1/products/:slug`, `POST/PUT/DELETE /v1/products` for admin)
- **OrdersModule** — Order management (`POST /v1/orders`, `GET /v1/orders`, `PATCH /v1/orders/:id/status` for admin)
- **ArticlesModule** — Blog articles and content (`GET /v1/articles`, `GET /v1/articles/:slug` for public, `POST/PUT/DELETE /v1/articles` for admin)
- **MediaModule** — Media uploads and management (`POST /v1/media/upload`, `GET /v1/media`, `DELETE /v1/media/:id` for admin, uses Cloudinary)
- **DashboardModule** — Admin dashboard with statistics (`GET /v1/dashboard/stats?period=today|week|month|all` for admin)
