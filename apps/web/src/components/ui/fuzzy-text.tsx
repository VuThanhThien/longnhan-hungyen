...
type CleanupCanvas = HTMLCanvasElement & { cleanupFuzzyText?: () => void };
...
function toCanvasText(children: ReactNode) {
...
export default function FuzzyText({
  ...
  const canvasRef = useRef<CleanupCanvas | null>(null);
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      raf = window.requestAnimationFrame(() => setResizeNonce((v) => v + 1));
    };
  ...
  useEffect(() => {
    ...
    // `@types/node` can change `setTimeout` return types; this component is browser-only.
    ...
    const init = async () => {
      ...
      const text = toCanvasText(children);
      ...
      // Reset transform to avoid cumulative translate across effect re-inits.
    ...
    return () => {
      ...
      if (glitchEndTimeoutId) window.clearTimeout(glitchEndTimeoutId);
      if (clickTimeoutId) window.clearTimeout(clickTimeoutId);
      if (canvasEl?.cleanupFuzzyText) canvasEl.cleanupFuzzyText();
    };
  }, [
    reducedMotion,
    resizeNonce,
