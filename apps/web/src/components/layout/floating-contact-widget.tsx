...
export default function FloatingContactWidget() {
  return (
    <>
      ...
      <div className="fixed left-4 bottom-24 z-50 hidden flex-col gap-2 md:flex">
        {contacts.map((c) => (
          <a
            ...
            className="flex items-center gap-2 rounded-full border border-(--brand-gold)/40 bg-(--brand-cream) px-3 py-2 text-xs font-semibold text-(--brand-forest) shadow-md transition hover:bg-(--brand-gold)/10"
          >
            {c.icon}
            <span>{c.label}</span>
          </a>
      ...
      <div className="fixed bottom-0 inset-x-0 z-50 flex justify-around border-t border-(--brand-gold)/25 bg-(--brand-cream) py-2 shadow-lg md:hidden"
