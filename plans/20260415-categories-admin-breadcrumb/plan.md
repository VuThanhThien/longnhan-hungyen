...
3. **Admin:** **No SEO** focus; prioritize **correct domain logic**, **TipTap** support for **editing images and image links**, **row navigation** on articles/products lists, **order detail** with **Save-before-API**, **validated** status transitions, **badges** mapped from enums to Vietnamese labels.
...
- TipTap: [`simple-editor.tsx`](../../apps/admin/src/components/tiptap-templates/simple/simple-editor.tsx) includes `@tiptap/extension-image` and **Link** via StarterKit — **no dedicated image URL editor / image-as-link** UX (need bubble menu or toolbar popover for selected image).
...
- Common approach: **`Image.extend`** with custom attributes (`href`), or **NodeView** with edit affordance; community pattern: **Image + Link** on double-click opens popover.
- Ensure **HTML serialization** matches what [`articles` API](../../apps/api/src/api/articles/) expects (`descriptionHtml` / body field — verify entity).
