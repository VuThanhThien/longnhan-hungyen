 ...
 *
 * Blocks access to directories listed in .claude/config/adf-ignore.txt
 ...
 *
 * Configuration:
 * - Edit .claude/config/adf-ignore.txt to customize blocked patterns (one per line, # for comments)
 ...
 * - 2: Command blocked
 *
 * Core logic extracted to lib/scout-checker.cjs for reuse.
 */
...
try {
  ...
  try {
    ...
    const result = checkScoutBlock({
      ...
      options: {
        claudeDir,
        adfIgnorePath: path.join(claudeDir, 'config', 'adf-ignore.txt'),
