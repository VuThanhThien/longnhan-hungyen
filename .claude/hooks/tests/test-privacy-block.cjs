...
const CK_CONFIG_PATH = path.join(__dirname, '..', 'config', 'adf-config.json');
...
async function main() {
  ...
  for (const test of blockTests) {
    ...
    const containsOk =
      !test.expectContains || result.stderr.includes(test.expectContains);
  ...
  // Config toggle tests - requires temp directory with .claude/config/adf-config.json
  ...
  const tmpConfigDir = path.join(tmpClaudeDir, 'config');
  fs.mkdirSync(tmpConfigDir, { recursive: true });

  for (const test of configToggleTests) {
    // Write test config
    fs.writeFileSync(
      path.join(tmpConfigDir, 'adf-config.json'),
