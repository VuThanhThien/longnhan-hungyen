 ...
 * Uses real adf-ignore fixtures to validate end-to-end behavior.
 *
...
const DEFAULT_OPTS = {
  ...
  adfIgnorePath: path.join(FIXTURES_DIR, 'adf-ignore-default.txt'),
...
describe('stripCommandPrefix', () => {
  it('strips single ENV var', () => {
    assert.strictEqual(
      stripCommandPrefix('NODE_ENV=production npm run build'),
      'npm run build',
    );
  });
  ...
  it('strips multiple ENV vars', () => {
    assert.strictEqual(
      stripCommandPrefix('NODE_ENV=prod CI=true npm run build'),
      'npm run build',
    );
  });
  ...
  it('strips env wrapper', () => {
    assert.strictEqual(
      stripCommandPrefix('env npm run build'),
      'npm run build',
    );
  });
  ...
  it('strips sudo + env vars', () => {
    assert.strictEqual(
      stripCommandPrefix('sudo NODE_ENV=prod npm start'),
      'npm start',
    );
  });
...
describe('unwrapShellExecutor', () => {
  ...
  it('returns original for non-executor', () => {
    assert.strictEqual(
      unwrapShellExecutor('cat node_modules/file'),
      'cat node_modules/file',
    );
  });
...
describe('tool command matching (via isBuildCommand)', () => {
  ...
  it('matches esbuild src/index.ts', () =>
    assert.ok(isBuildCommand('esbuild src/index.ts')));
  ...
  it('matches prettier --check', () =>
    assert.ok(isBuildCommand('prettier --check .')));
  it('matches go build', () =>
    assert.ok(isBuildCommand('go build ./cmd/server')));
  it('matches cargo build', () =>
    assert.ok(isBuildCommand('cargo build --release')));
  it('matches make', () => assert.ok(isBuildCommand('make')));
  it('matches docker build', () =>
    assert.ok(isBuildCommand('docker build -t app .')));
  it('matches kubectl', () =>
    assert.ok(isBuildCommand('kubectl apply -f deploy.yaml')));
  it('matches terraform', () => assert.ok(isBuildCommand('terraform plan')));
  it('matches turbo run build', () =>
    assert.ok(isBuildCommand('turbo run build')));
...
describe('checkScoutBlock - fail-open behavior', () => {
  ...
  it('uses defaults when adfIgnorePath missing', () => {
    const r = checkScoutBlock({
      toolName: 'Bash',
      toolInput: { command: 'ls node_modules' },
      options: { adfIgnorePath: '/nonexistent/adf-ignore.txt' },
    });
    assert.ok(r.blocked, 'Should still block using DEFAULT_PATTERNS');
  });
});

describe('checkScoutBlock - custom adf-ignore.txt', () => {
  const customOpts = {
    ...DEFAULT_OPTS,
    adfIgnorePath: path.join(FIXTURES_DIR, 'adf-ignore-custom.txt'),
  };
...
describe('checkScoutBlock - negation adf-ignore.txt', () => {
  const negOpts = {
    ...DEFAULT_OPTS,
    adfIgnorePath: path.join(FIXTURES_DIR, 'adf-ignore-negation.txt'),
  };
...
describe('P0 - absolute and relative paths', () => {
  ...
  it('blocks cat with absolute node_modules path', () => {
    const r = checkScoutBlock({
      toolName: 'Bash',
      toolInput: {
        command: 'cat /home/user/project/node_modules/pkg/index.js',
      },
      options: DEFAULT_OPTS,
    });
    assert.ok(r.blocked);
