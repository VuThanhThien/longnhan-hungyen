...
const testCases = [
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: '.venv/Scripts/python.exe script.py' },
    },
    expected: 'ALLOWED',
  },
  {
  ...
  // venv (without dot)
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'venv/bin/python3 script.py' },
    },
    expected: 'ALLOWED',
  },
  {
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: './myproject/venv/bin/pip install flask' },
    },
    expected: 'ALLOWED',
  },
  ...
  // Venv exploration - should be BLOCKED
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'cat .venv/lib/python3.11/site.py' },
    },
    expected: 'BLOCKED',
  },
  {
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'py -3.11 -m venv .venv' },
    },
    expected: 'ALLOWED',
  },
  {
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'virtualenv --python=python3.11 my_env' },
    },
    expected: 'ALLOWED',
  },
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'echo "Building..." && npm run build' },
    },
    expected: 'ALLOWED',
  },
  {
  ...
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'echo "test" && cd node_modules' },
    },
    expected: 'BLOCKED',
  },
  {
    ...
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'npm run build && cat dist/bundle.js' },
    },
    expected: 'BLOCKED',
  },
...
for (const test of testCases) {
  try {
    ...
    if (success) {
      ...
      console.log(
        `\x1b[31m✗\x1b[0m ${test.name}: expected ${test.expected}, got ${actual}`,
      );
      failed++;
    }
  } catch (error) {
    ...
    if (success) {
      ...
      if (error.stderr) {
        console.log(
          `  Error: ${error.stderr.toString().trim().split('\n')[0]}`,
        );
      }
      failed++;
    }
