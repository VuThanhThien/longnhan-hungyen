...
describe('Issue #327: Path Resolution Integration', () => {
  describe('Subdirectory Workflow', () => {
    it('all hooks use CWD as base when in subdirectory', async () => {
      ...
      assert.strictEqual(
        devRulesResult.exitCode,
        0,
        'dev-rules-reminder should succeed',
      );

      // Verify subagent output includes subdirectory path
      const subagentContext =
        subagentResult.output?.hookSpecificOutput?.additionalContext || '';
      assert.ok(
        subagentContext.includes(subdirPath) ||
          subagentContext.includes('.claude/hooks'),
    ...
    it('handles non-git directory gracefully', async () => {
      ...
      try {
        ...
        assert.strictEqual(
          sessionResult.exitCode,
          0,
          'session-init should succeed without git',
        );
        ...
        assert.strictEqual(
          subagentResult.exitCode,
          0,
          'subagent-init should succeed without git',
        );
      } finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
      }
    ...
    it('all hooks fail-open on errors', async () => {
      ...
      for (const hookName of hooks) {
        ...
        assert.strictEqual(
          exitCode,
          0,
          `${hookName} should fail-open with exit 0`,
        );
      }
    });
  });
