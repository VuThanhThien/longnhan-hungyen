...
describe('team-context-inject.cjs', () => {
  describe('Basic Functionality', () => {
    ...
    it('exits 0 on empty stdin', async () => {
      ...
      assert.strictEqual(
        result.exitCode,
        0,
        'Should exit cleanly on empty stdin',
      );
    });
  ...
  describe('Team Config Loading', () => {
    ...
    it('silently exits if team config does not exist', async () => {
      const tmpDir = path.join(
        os.tmpdir(),
        'team-inject-no-config-' + Date.now(),
      );
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        // Set up .claude/teams directory but no team-a
        fs.mkdirSync(path.join(tmpDir, '.claude', 'teams'), {
          recursive: true,
        });
        ...
        assert.strictEqual(
          result.exitCode,
          0,
          'Should exit cleanly when config missing',
        );
    ...
    it('handles corrupted JSON config gracefully', async () => {
      ...
      try {
        ...
        assert.strictEqual(
          result.exitCode,
          0,
          'Should fail-open on corrupt JSON',
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
  ...
  describe('CK Stack Context Building', () => {
    it('includes CK context when environment variables are set', async () => {
      ...
      try {
        ...
        assert.ok(
          context.includes('CK Context'),
          'Should have CK Context section',
        );
        assert.ok(
          context.includes('/project/plans/reports'),
          'Should include reports path',
        );
        assert.ok(
          context.includes('/project/plans'),
          'Should include plans path',
        );
  ...
  describe('Task Summary Generation', () => {
    ...
    it('handles corrupted task files gracefully', async () => {
      const tmpDir = path.join(
        os.tmpdir(),
        'team-inject-bad-tasks-' + Date.now(),
      );
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        ...
        assert.strictEqual(
          result.exitCode,
          0,
          'Should fail-open on corrupt tasks',
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    ...
    it('counts only .json files as tasks', async () => {
      const tmpDir = path.join(
        os.tmpdir(),
        'team-inject-json-filter-' + Date.now(),
      );
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        const taskDir = path.join(tmpDir, '.claude', 'tasks', 'team-a');
  ...
  describe('JSON Output Validation', () => {
    it('always outputs valid JSON with hookSpecificOutput structure', async () => {
      const tmpDir = path.join(
        os.tmpdir(),
        'team-inject-json-valid-' + Date.now(),
      );
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        createTestTeam(tmpDir, 'team-a');
