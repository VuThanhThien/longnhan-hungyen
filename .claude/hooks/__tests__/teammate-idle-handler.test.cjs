  ...
  describe('Task availability detection', () => {
    ...
    it('suggests shutdown when no tasks remain', async () => {
      ...
      try {
        ...
        const ctx = result.output.hookSpecificOutput.additionalContext;
        assert.ok(
          ctx.includes('No remaining tasks'),
    ...
    it('skips tasks that have an owner (already claimed)', async () => {
      ...
      try {
        ...
        assert.ok(
          !ctx.includes('#1 "Claimed"'),
          'Should not list owned task as available',
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    ...
    it('correctly identifies unblocked tasks (blockedBy all completed)', async () => {
      ...
      try {
        ...
        const ctx = result.output.hookSpecificOutput.additionalContext;
        assert.ok(
          ctx.includes('Unblocked now'),
          'Should list task whose deps are met',
        );
        assert.ok(
          !ctx.includes('Still blocked'),
          'Should not list task with unmet deps',
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
