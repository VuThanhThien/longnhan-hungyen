  ...
  function main() {
    try {
      ...
      if (taskInfo) {
        ...
        lines.push(
          `Tasks: ${taskInfo.completed}/${taskInfo.total} done. ${remaining} remaining.`,
        );
        ...
        if (taskInfo.unblocked.length > 0) {
          ...
          lines.push(
            `Consider assigning work to ${teammate_name} or waking them with a message.`,
          );
        } else if (remaining === 0) {
          lines.push(
            `No remaining tasks. Consider shutting down ${teammate_name}.`,
          );
        } else {
          lines.push(
